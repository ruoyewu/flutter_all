...
abstract class AppPage extends StatelessWidget {
				...
				if (constraints.maxWidth < 500) {
					return _buildPage(item, (item) {
						Navigator.pushNamed(context, UIData.ROUTE_ARTICLE_DETAIL,
							arguments: {'item': item, 'hero': heroTag, 'title': item.title});
					});
				} else {
					return Stack(
						children: <Widget>[
	...
	List<Widget> buildPages (Type type, AppItem item,
		...
		return CupertinoPageScaffold(
			child: CustomScrollView(
				slivers: <Widget>[
					...
					item.channel.length == 1 ? AppListWidget(
						Type.CUPRETINO,
						channel: item.channel[0],
						onArticleItemTap: onArticleItemTap,
					) : AppChannelListWidget(Type.CUPRETINO, item, onArticleItemTap: onArticleItemTap,),
				],
			),
		);
	}
